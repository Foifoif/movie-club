-- Replace the existing resolver with the same bracket flow, qualifying the
-- two bracket_votes.entry_id references that collide with the PL/pgSQL variable.
...
begin
  ...
  if p_reason = 'timer' and vote_count < 3 then
    update public.bracket_matchups set closes_at = coalesce(closes_at, now()) + interval '24 hours'
  ...
  for entry_id in select bm.winner_entry_id from public.bracket_matchups bm
    ...
    if pending_id is null then pending_id := entry_id; else
      ...
      values (matchup.round_id, next_round, pending_id, entry_id, 'OPEN', next_open, next_open + interval '24 hours');
  ...
  if next_matchups = 0 and pending_id is null then
    ...
    update public.round_phases set opens_at = next_open, closes_at = next_open + interval '24 hours' where id = phase_id;
